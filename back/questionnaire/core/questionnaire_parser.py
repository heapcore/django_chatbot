...
class QuestionnaireParser:
    ...
    def __save_node(self, tree):
        ...
        question_from = QuestionAnswer.objects.create(
            name=tree["text"], is_leaf=is_leaf
        )

        if not is_leaf:
            for response in tree["response"]:
                QuestionAnswerRelation.objects.create(
                    condition=response,
                    question_from_id=question_from,
    ...
    def validate(self):
        ...
        if questionnaire_exists:
            is_valid = False
            message = (
                "Cannot save the questionnaire '{name}', because "
                "the questionnaire with the same name already exists"
        ...
        if unallowable_question:
            ...
            message = (
                "Cannot save the questionnaire '{name}', because "
                "the question '{question}' has more than 5 answers"
            ).format(name=self.name, question=unallowable_question)

        return is_valid, message

    def save(self):
        Questionnaire.objects.create(
            name=self.name, start_question_id=self.__save_node(self.tree)
        )
